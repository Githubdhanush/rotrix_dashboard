...
logo_base64 = get_base64_image("C:/Users/Dhanu/Downloads/Rotrix-Logo.png")
...
#             st.info("No data uploaded yet.")
    ...
    #                 color_axis = st.selectbox("Color by (Optional)", ["None"] + common_cols)
        ...
        #             with col3:
            ...
            # --- Single file analysis for Benchmark ---
            df = st.session_state.b_df
            st.subheader("🧪 Benchmark Data Analysis")
            ...
            if not numeric_columns:
                ...
                col1, col2, col3 = st.columns([0.20, 0.60, 0.20])
                with col1:
                    x_axis = st.selectbox("Select X-Axis", numeric_columns, key="single_x_axis_bench")
                    y_axis = st.selectbox("Select Y-Axis", [col for col in numeric_columns if col != x_axis], key="single_y_axis_bench")
                with col2:
                    if x_axis and y_axis:
                        stats = df[y_axis].describe()
                        st.write(stats)
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=df[x_axis], y=df[y_axis], mode="lines", name="Data"))
                        try:
                            z = np.polyfit(range(len(df)), df[y_axis], 1)
                            p = np.poly1d(z)
                            fig.add_trace(go.Scatter(x=df[x_axis], y=p(range(len(df))), mode="lines", name="Trend Line", line=dict(dash="dash")))
                        except Exception as e:
                        ...
                        st.plotly_chart(fig, use_container_width=True)
                # col3 can be left empty or used for future metrics
        elif st.session_state.v_df is not None:
            # --- Single file analysis for Target ---
            df = st.session_state.v_df
            st.subheader("🔬 Target Data Analysis")
            numeric_columns = df.select_dtypes(include=[np.number]).columns.tolist()
            if not numeric_columns:
                st.warning("No numeric columns found for visualization")
            else:
                col1, col2, col3 = st.columns([0.20, 0.60, 0.20])
                with col1:
                    x_axis = st.selectbox("Select X-Axis", numeric_columns, key="single_x_axis_val")
                    y_axis = st.selectbox("Select Y-Axis", [col for col in numeric_columns if col != x_axis], key="single_y_axis_val")
                with col2:
                    if x_axis and y_axis:
                        stats = df[y_axis].describe()
                        st.write(stats)
            ...
            st.info("Please upload benchmark or validation files.")
